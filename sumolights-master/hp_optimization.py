...
def parse_cl_args():
    ...
    parser.add_argument("-tsc", type=str, default='websters', dest='tsc', help='traffic signal control algorithm, default:websters; options:sotl, maxpressure, dqnx, ddpg'  )
...
def get_hp_dict(tsc_str):
    if tsc_str == 'dqnx':
        # return {'-lr':[ 0.0001, 0.00005], '-lre':[0.000001, 0.0000001, 0.00000001], '-updates':[15000], '-batch':[64], '-nreplay':[15000], '-nsteps':[1,2], '-n_hidden':[3, 6], '-target_freq':[32,64,128], '-gmin':[5,10]}
        # next try tuning

        return {'-lr':[0.0001], '-lre':[0.0000001], '-updates':[60000, 120000], '-batch':[32], '-nreplay':[100000], '-n_layers':[3],'-nsteps':[2], '-target_freq':[32], '-gmin':[12]}

        # this setting doesn't work since the the nreplay is too big and there 
        # isn't that many experiences
        # also the 12 hidden layers are too many for the machine to handle
        # so the run time is pretty high
...
def create_hp_cmds(args, hp_order, hp):
    ...
    if args.tsc == 'ddpg' or args.tsc == 'dqnx':
