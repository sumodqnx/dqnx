...
class TrafficMetrics:
    ...
    def get_special_reward(self):
        return self.metrics["queue"].get_square_queue()
...
class QueueMetric(TrafficMetric):
    ...
    #     return max([self.lane_queues[lane] for lane in self.lane_queues])

    def get_square_queue(self):
        return int(sum([self.lane_queues[lane]**2 for lane in self.lane_queues]))
