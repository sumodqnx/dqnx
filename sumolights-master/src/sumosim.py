...
class SumoSim:
    ...
    def gen_sim(self):
        ...
        # place to generate incident: tianxin
        self.incident = None
        if self.args.incident:
            self.incident = Incident(self.netdata, 
                                    self.args.sim_len, 
                                    self.conn)
            
            # print("total amount of vehicles in the simulation")
            # print(self.vehiclegen.vehicles_created)
    ...
    def update_netdata(self):
        ...
        tsc = { tl:TrafficSignalController(self.conn, tl, self.args.mode, self.netdata, self.args.y, self.args.r)  
                     for tl in tl_junc }

        for t in tsc:
            self.netdata['inter'][t]['incoming_lanes'] = tsc[t].incoming_lanes
            #tianxin added begin
            self.netdata['inter'][t]['outgoing_lanes'] = tsc[t].outgoing_lanes
            #tianxin added end
    ...
    def sim_step(self):
        ...
        #tianxin: add incident car begins

        if self.incident and self.t == self.incident.incident_time:
            self.incident.set_incident()

        if self.incident and self.t == self.incident.emergency_time:
            self.incident.set_emergency()
        #tianxin: add emergency car ends
        self.t += 1
    ...
    def run(self):
        ...
        #tianxin: output the demand for each simulation
        with open('demand.txt', 'a') as f:
            f.write(str(self.vehiclegen.vehicles_created) + '\n')
